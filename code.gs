...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem('📢 Exportar pacientes para campaña', 'exportarPacientesParaCampana')
...
function setupSheets() {
  ...
  const definicion = {
    'CITAS': {
      headers: ['ID', 'Fecha', 'Hora', 'Doctor', 'Especialidad', 'Paciente',
                'Teléfono', 'Servicio', 'Precio (₡)', 'Estado', 'Notas',
                'En Calendar', 'Event ID', 'Fecha Creación'],
      anchos: [80, 100, 70, 160, 140, 180, 110, 180, 100, 110, 200, 90, 120, 120]
    },
    'DOCTORES': {
      headers: ['Nombre', 'Especialidad', 'Teléfono', 'Calendar ID', 'Usa Calendar (Sí/No)', 'Color'],
  ...
  if (shDoc.getLastRow() <= 1) {
    shDoc.getRange(2, 1, 3, 6).setValues([
      ...
      ['Dr. Ramírez Solís', 'Medicina General', '8888-3333', '', 'Sí', '#ea4335'],
    ]);
  }
...
function getDoctores() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SHEET_DOCTORES);
...
function getServiciosPorDoctor(nombreDoctor) {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SHEET_SERVICIOS);
  if (!sheet || sheet.getLastRow() <= 1) return [];
...
function getPacientesSugeridos(query) {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SHEET_PACIENTES);
  if (!sheet || sheet.getLastRow() <= 1) return [];
  const q = String(query).toLowerCase();
  return sheet.getRange(2, 1, sheet.getLastRow() - 1, 2).getValues()
    .filter(r => r[0] && (
      String(r[0]).toLowerCase().includes(q) ||
      String(r[1]).includes(q)
    ))
...
function getCitas(filtros) {
  filtros = filtros || {};
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SHEET_CITAS);
  if (!sheet || sheet.getLastRow() <= 1) return [];
  ...
  let citas = data
    .map((row, i) => {
      ...
      if (fechaRaw instanceof Date && !isNaN(fechaRaw)) {
        fechaStr = Utilities.formatDate(fechaRaw, 'America/Costa_Rica', 'yyyy-MM-dd');
      ...
      return {
        ...
        id:          row[0],
        fecha:       fechaStr,
        hora: row[2] instanceof Date ? Utilities.formatDate(row[2], 'America/Costa_Rica', 'HH:mm') : (row[2] ? String(row[2]).substring(0, 5) : ''),
        doctor:      row[3],
        especialidad: row[4],
        paciente:    row[5],
        telefono:    row[6],
...
function agregarCita(cita) {
  try {
    ...
    const row = [
      id, fecha, cita.hora, cita.doctor, especialidad,
      cita.paciente, cita.telefono, cita.servicio, cita.precio,
      'Pendiente', cita.notas || '', 'No', '', ahora
    ];
...
function actualizarEstadoCita(rowIndex, nuevoEstado) {
  try {
    ...
    sheet.getRange(rowIndex, 1, 1, 14).setBackground(colores[nuevoEstado] || '#ffffff');
    return { success: true };
  } catch (e) {
    return { success: false, error: e.message };
  }
...
function guardarPaciente(nombre, telefono) {
  try {
    ...
    const existe = data.some(r =>
      String(r[0]).toLowerCase() === String(nombre).toLowerCase()
    );
    ...
    if (!existe && nombre) {
      ...
      if (idx !== -1) {
        ...
        sheet.getRange(idx + 2, 5).setValue(
          Utilities.formatDate(new Date(), 'America/Costa_Rica', 'yyyy-MM-dd')
        );
      }
    }
...
function generarMensajeWhatsApp(cita, tipo) {
  ...
  : String(cita.especialidad || '')
      ...
      .replace('Fisiatría', 'Fisiatr.') + '/' + cita.servicio;

const precioFormato = String(Number(cita.precio || 0))
  .replace(/\B(?=(\d{3})+(?!\d))/g, '.');
...
function verificarRecordatorios() {
  ...
  const lineas = citasManana.map(c =>
...
function registrarLogDiario(citas, fecha) {
  ...
  if (!logSheet) {
    ...
    logSheet.getRange(1,1,1,3).setValues([['Fecha','Hora registro','Citas mañana']]);
    logSheet.getRange(1,1,1,3).setBackground('#5f6368').setFontColor('#fff').setFontWeight('bold');
  }
...
function mostrarResumenHoy() {
  ...
  const resumen = citas.map(c =>
    `${c.hora}  ${c.paciente}  →  ${c.doctor}  [${c.estado}]`
  ).join('\n');

  SpreadsheetApp.getUi().alert(`📋 Citas de hoy (${hoy}) — Total: ${citas.length}\n\n${resumen}`);
}
...
function importarCitasDesdeCalendar() {
  ...
  const existentes = sheet.getLastRow() > 1
    ? sheet.getRange(2, 13, sheet.getLastRow() - 1, 1)
  ...
  doctores.forEach(doctor => {
    try {
      ...
      eventos.forEach(evento => {
        ...
        const paciente = extraerCampo(desc, 'Paciente') || tituloPartes[0] || evento.getTitle();
        const telefono = extraerCampo(desc, 'Teléfono') || '';
        const servicio = extraerCampo(desc, 'Servicio') || tituloPartes[1] || '';
        ...
        const fecha   = Utilities.formatDate(inicio, 'America/Costa_Rica', 'yyyy-MM-dd');
        const hora    = Utilities.formatDate(inicio, 'America/Costa_Rica', 'HH:mm');
        const id      = 'CAL' + new Date().getTime() + Math.random().toString(36).slice(2,5);

        const fila = [
          id, new Date(fecha + 'T12:00:00-06:00'), hora,
          doctor.nombre, doctor.especialidad,
          paciente, telefono, servicio,
          precio.replace(/[₡,]/g, '') || 0,
          'Confirmada', '', 'Sí', eventId, new Date()
        ];
...
function sincronizarContactoWhatsApp(nombre, telefono) {
  ...
  try {
    ...
    Logger.log('❌ Error contacto: ' + e.message + ' | ' + nombre + ' | ' + telefono);
  }
}
...
function exportarPacientesParaCampana() {
  ...
  const pacientes = shPacientes.getRange(2, 1, shPacientes.getLastRow() - 1, 5).getValues()
  ...
  expSheet.getRange(1, 1, 1, 5).setValues([['Nombre', 'Teléfono', 'Teléfono Internacional', 'Link WhatsApp', 'Lote']])
  ...
  const filas = pacientes.map((p, i) => {
    ...
    const telIntl = telLimpio
      ? (telLimpio.startsWith('+') ? telLimpio : '+506' + telLimpio)
      : '';
    const lote = i < 256 ? 'Lista 1' : 'Lista 2';
    return [p[0], p[1], telIntl, telIntl ? 'https://wa.me/' + telIntl.replace('+', '') : '', lote];
  });
...
function limpiarYExportarCampana() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const shPacientes = ss.getSheetByName(CONFIG.SHEET_PACIENTES);
  const datos = shPacientes.getRange(2, 1, shPacientes.getLastRow() - 1, 2).getValues();

  // Palabras que indican que NO es un paciente
  const basura = [
    'no agendar','no citar','feriado','feria','agenda','consulta',
    'almuerzo','fotos','citas de','interview','psicología','spa',
    'suarez','teresita','domicilio','taller','zoom','virtual',
    'fuera','lista de espera','mayra','it set','deel','dr.sáenz',
    'para don','para idaly','tratamiento domicilio','no trabajo',
    'agendar 7','agenda de 7','agenda para','no angendar'
  ];
  ...
  expSheet.getRange(1, 1, 1, 6)
    .setValues([['Nombre', 'Teléfono', 'Teléfono Internacional', 'Link WhatsApp', 'Lote', 'Estado']])
    .setBackground('#1a73e8').setFontColor('#fff').setFontWeight('bold');
...
function protegerHojaPacientes() {
  ...
  SpreadsheetApp.getUi().alert('✅ Validación aplicada. La columna Teléfono solo aceptará números.');
}
...
// ── Intercepción de reserva pendiente ───────────────────────
  const sistemaReserva = historial.filter(function(h) { 
  ...
  if (sistemaReserva) {
    ...
    const horaEnMensaje = mensaje.match(/\b([0-9]{1,2})[:\.]([0-9]{2})\b/) ||
    ...
    if (ctx.includes('esperando=nombre') && ctxDoctor && ctxFecha && ctxHora) {
      const nombresInvalidos = ['si','sí','no','ok','okay','bien','hola','gracias','nada'];
      const nombreLimpio = mensaje.trim();

      if (nombreLimpio.length > 2 && !nombresInvalidos.includes(nombreLimpio.toLowerCase())) {
        const servicios2 = getTodosLosServicios();
        const servicio2  = servicios2.find(function(s) { return s.doctor === ctxDoctor; });

        // Limpiar teléfono a 8 dígitos
        const telLimpio2 = String(telefono).replace(/[^0-9]/g, '').slice(-8);
        ...
        const respuesta = resultado.success
          ? 'Su cita quedó agendada con ' + ctxDoctor + ' el ' + ctxFecha + ' a las ' + ctxHora + '. Le esperamos en ' + CONFIG.NOMBRE_CLINICA + '.'
    ...
    if (horaEnMensaje && slots.length > 0 && ctxDoctor && ctxFecha) {
      ...
      if (m1)      { hh = m1[1]; mm = m1[2]; }
      else if (m2) { hh = m2[1]; mm = '00';  }
      ...
      if (slots.indexOf(horaNorm) !== -1) {
        ...
        const respuesta = 'Con gusto le agendo el ' + ctxFecha + ' con ' + ctxDoctor + ' a las ' + horaNorm + '. ¿Me confirma su nombre completo?';
        guardarHistorial(telefono, 'user', mensaje);
        guardarHistorial(telefono, 'assistant', respuesta);
        guardarHistorial(telefono, 'system', 'CONTEXTO_RESERVA: doctor=' + ctxDoctor + ' | fecha=' + ctxFecha + ' | hora=' + horaNorm + ' | esperando=nombre');
        return respuesta;
      } else {
        // Hora no disponible — mostrar alternativas
        const respuesta = 'Lo sentimos, las ' + horaNorm + ' no está disponible. Las horas libres son:\n\n' +
          slots.map(function(h) { return '⏰ ' + h; }).join('\n') + '\n\n¿Cuál le queda mejor?';
        guardarHistorial(telefono, 'user', mensaje);
        guardarHistorial(telefono, 'assistant', respuesta);
        return respuesta;
      }
  ...
  const contextoCitas = citasPaciente.length > 0
    ? citasPaciente.slice(0,5).map(c =>
  ...
  const contextoServicios = servicios.length > 0
    ? servicios.map(s =>
  ...
  const contextoDoctores = doctores.map(d =>
  ...
  // Acción: agendar cita nueva
    if (json.accion === 'agendar') {
      ...
      const saludo   = esInicio ? 'Buen día, soy Maureen de Clínica Integral Mediveins. ' : '';

      // Validar que el nombre del paciente sea real
      const nombresInvalidos = ['pendiente','cliente','paciente','usuario',
                                'jimena','nombre','desconocido','','null','undefined'];
      const nombreValido = json.paciente &&
        !nombresInvalidos.includes(String(json.paciente).toLowerCase().trim()) &&
        String(json.paciente).trim().length > 2;

      if (!nombreValido) {
        // No tenemos nombre real — pedirlo antes de agendar
        const horaNorm = json.hora || '';
        const respuesta = `${saludo}Con gusto le agendo para el ${json.fecha} con ${json.doctor}` +
      ...
      if (json.fecha && json.hora && json.doctor) {
        ...
        const horaOcupada = disp.ocupados.some(h => h.substring(0,5) === horaNorm);

        if (horaOcupada) {
          const slots = disp.disponibles.slice(0, 6);
          const respuesta = slots.length > 0
...
// Limpiar teléfono — preferir el que dio el paciente, si no usar el del WhatsApp
      const telPaciente = json.telefono
        ? String(json.telefono).replace(/[^0-9]/g, '').slice(-8)
        : String(telefono).replace(/[^0-9]/g, '').slice(-8);
      ...
      const respuesta = resultado.success
        ? (saludo + (json.texto || `Su cita quedó agendada con ${json.doctor} el ${json.fecha} a las ${json.hora}.`))
   ...
   // Acción: disponibilidad
    if (json.accion === 'disponibilidad' && json.doctor && json.fecha) {
      ...
      const horaMatch = mensaje.match(/\b([0-9]{1,2})[:\s\.]([0-9]{2})\b/) ||
      ...
      if (slots.length === 0) {
        ...
        respuesta = `Para el ${json.fecha} con ${json.doctor} tenemos estos espacios:\n\n` +
                   slots.map(h => `⏰ ${h}`).join('\n') +
                   `\n\n¿Cuál le queda mejor?`;
      }
      // Guardar contexto de reserva pendiente para el siguiente mensaje
      guardarHistorial(telefono, 'assistant', respuesta);
      guardarHistorial(telefono, 'system', `CONTEXTO_RESERVA: doctor=${json.doctor} | fecha=${json.fecha} | slots_disponibles=${slots.join(',')}`);
      return respuesta;
    }
    ...
    if (json.accion === 'confirmar' && json.rowIndex) {
      ...
      const respuesta = json.texto || '✅ Perfecto, su cita quedó confirmada. ¡Le esperamos!';
      guardarHistorial(telefono, 'assistant', respuesta);
      return respuesta;
    }
    ...
    if (json.accion === 'cancelar' && json.rowIndex) {
      ...
      const respuesta = json.texto || 'Su cita fue cancelada. Si desea reagendarla con gusto la ayudo.';
      guardarHistorial(telefono, 'assistant', respuesta);
      return respuesta;
    }
    ...
    const fallback = 'Disculpe, tuve un problema. ¿Me puede repetir lo que necesita?';
...
function enviarWhatsApp(para, mensaje) {
  ...
  const url = 'https://api.twilio.com/2010-04-01/Accounts/' +
               config.twilioSid + '/Messages.json';
...
function getHistorial(telefono) {
  ...
  if (!sheet) {
    ...
    sheet.getRange(1,1,1,4).setValues([['Teléfono','Timestamp','Rol','Mensaje']]);
    sheet.getRange(1,1,1,4).setBackground('#1a73e8').setFontColor('#fff').setFontWeight('bold');
  }
...
function guardarHistorial(telefono, rol, mensaje) {
  ...
  sheet.appendRow([
    telefono,
    Utilities.formatDate(new Date(), 'America/Costa_Rica', 'yyyy-MM-dd HH:mm:ss'),
    rol,
    mensaje
  ]);
}
...
function getDisponibilidad(nombreDoctor, fecha) {
  ...
  const disponibles = todosLosHorarios.filter(h => !citasOcupadas.includes(h));
...
function enviarRecordatoriosWhatsApp() {
  ...
  const basura = ['almuerzo','feriado','agenda','citas de','domicilio',
                'no agendar','zoom','virtual','fuera','taller','it set'];
...
if (!/^\d{8}$/.test(telLimpio)) {
  Logger.log('⚠️ Teléfono inválido, saltando: ' + cita.paciente + ' · "' + cita.telefono + '"');
...
const telIntl = '+506' + telLimpio;
      ...
      if (twilioData.error_code || twilioData.code) {
        Logger.log('❌ Error Twilio para ' + cita.paciente + ': ' + twilioData.message);
      } else {
        Logger.log('✅ Recordatorio enviado a: ' + cita.paciente + ' · ' + para);
      }
...
function configurarTriggerRecordatoriosWhatsApp() {
  ...
  Logger.log('✅ Trigger configurado: recordatorios WhatsApp cada día a las 8am');
}
